...
class RedisCommonFunction{  
    public $redis_obj = null;//redis实例化时静态变量  
  
    public function __construct($host = '127.0.0.1', $port =6379,$auth = "password") {  
...
//          echo "$key - $value \n";  
     ...
     * @return int $number 插入成功返回1【,更新操作返回0】
     */
     ...
     * @param $token ,$id为活动ID
     * @param $id
     * @return array|bool , 成功返回要查询的信息，失败或不存在返回false
     * @internal param $redis ,$redis['key'],$redis['field']分别是hash的表名称和键值
     */
     ...
     * @return bool $number 返回OK【更新和插入操作都返回ok】
     */
